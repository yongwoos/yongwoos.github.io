...
전용 인스턴스와 유사하지만 물리 서버 선택이 가능

소켓, 코어, 메모리 등 하드웨어 리소스를 직접 제어 가능.

같은 계정 끼리는 하드웨어 공유 가능
...
EC2를 사용하다 Fargate로 자유롭게 전환 가능(시간당 약정 금액 내에서).
...
특정 인스턴스 패밀리에 묶이지만, 리전 내에서 인스턴스 타입, OS, 테넌시 변경 가능
