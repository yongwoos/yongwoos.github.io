...
Delivery Pipeline을 생성한다. Delivery Pipeline은 타깃의 순서를 정의한다. 예를 들어 staging -> production 순서로 배포할 수 있다.
스캐폴딩을 설정한다. 스캐폴딩은 애플리케이션의 빌드와 배포를 자동화하는 방법을 정의한다.
...
## 배포 과정
1.  target 생성
2.  delivery pipeline 생성
3.  scaffold 설정
4.  release 생성
5.  rollout 생성
6.  advance api 호출 (카나리 배포의 경우)
